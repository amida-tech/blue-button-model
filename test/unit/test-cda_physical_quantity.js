...
describe('cda_physical_quantity unit tests', function () {
    it('sample cda_physical_quantity_0', function () {
        bbm.validator.validateComponent(samples.valid_0, 'cda_physical_quantity');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.true;
    });

    it('sample invalid_0', function () {
        bbm.validator.validateComponent(samples.invalid_0, 'cda_physical_quantity');
        var result = bbm.validator.getLastError();
    ...
    it('sample invalid_1', function () {
        bbm.validator.validateComponent(samples.invalid_1, 'cda_physical_quantity');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.false;
    ...
    it('sample invalid_2', function () {
        bbm.validator.validateComponent(samples.invalid_2, 'cda_physical_quantity');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.false;
        expect(result.errors).to.have.length(1);
        var c2p = util.errorsToCodePathMap(result.errors);
    ...
    it('sample invalid_3', function () {
        bbm.validator.validateComponent(samples.invalid_3, 'cda_physical_quantity');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.false;
        expect(result.errors).to.have.length(1);
        var c2p = util.errorsToCodePathMap(result.errors);
