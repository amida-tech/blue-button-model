...
describe('cda_organization unit tests', function () {
    it('sample cda_organization_0', function () {
        bbm.validator.validateComponent(samples.valid_0, 'cda_organization');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.true;
    });

    it('sample valid_1', function () {
        bbm.validator.validateComponent(samples.valid_1, 'cda_organization');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.true;
    });

    it('sample invalid_0', function () {
        bbm.validator.validateComponent(samples.invalid_0, 'cda_organization');
        var result = bbm.validator.getLastError();
