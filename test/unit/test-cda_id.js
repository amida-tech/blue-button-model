...
describe('cda_id unit tests', function () {
    it('sample cda_id_0', function () {
        var valid = bbm.validator.validateComponent(samples.valid_0, 'cda_id');
        expect(valid).to.be.true;
    });

    it('sample valid_1', function () {
        var valid = bbm.validator.validateComponent(samples.valid_1, 'cda_id');
        expect(valid).to.be.true;
    });

    it('sample invalid_0', function () {
        var valid = bbm.validator.validateComponent(samples.invalid_0, 'cda_id');
    ...
    it('sample invalid_1', function () {
        var valid = bbm.validator.validateComponent(samples.invalid_1, 'cda_id');
        expect(valid).to.be.false;
        var result = bbm.validator.getLastError();
        expect(result.errors).to.have.length(1);
        var c2p = util.errorsToCodePathMap(result.errors);
