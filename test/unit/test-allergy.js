...
describe('allergy unit tests', function () {
    it('sample valid_0', function () {
        bbm.validator.validateComponent(samples.valid_0, 'allergy');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.true;
    });

    it('sample valid_1', function () {
        bbm.validator.validateComponent(samples.valid_1, 'allergy');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.true;
    });

    it('sample invalid_0', function () {
        bbm.validator.validateComponent(samples.invalid_0, 'allergy');
        var result = bbm.validator.getLastError();
    ...
    it('sample invalid_1', function () {
        bbm.validator.validateComponent(samples.invalid_1, 'allergy');
        var result = bbm.validator.getLastError();
        expect(result.valid).to.be.false;
        expect(result.errors).to.have.length(2);
        var c2p = util.errorsToCodePathMap(result.errors);
